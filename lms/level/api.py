...
from lms.level.parse import LevelParse
from lms.level.rpc import (
    RPC
)
...
class API(APIBase):
    ...
    ENDPOINT = 'http://lmsapi.webaula.com.br/v3/Level.svc?singleWsdl'
    ...
    def get_all(self, data_rq):
        ...
        data = LevelParse.get_all(response)

        # Retornar o course response

        data_rs = GetAllLevelRS(
            error=response['hasError'],
            guid=response['Guid'],
            msg=response['Msg'],
            data=data
